...
h1 = vector_plot(Sim_t,Sim_position_NED,MCU_position_NED,{'North (m)', 'East (m)', 'Down (m)', 'NED Position Review'},[-10 10]);
h2 = reference_plot(Sim_t,Sim_Euler,MCU_Euler,MCU_Euler_ref,{'Roll (rad)', 'Pitch (rad)', 'Yaw (rad)', 'Euler Angles Review'},[-pi pi]);
h3 = reference_plot(Sim_t,Sim_w,MCU_w,MCU_w_ref,{'w_x (rad/s)', 'w_y (rad/s)', 'w_z (rad/s)', 'Body Angular Rate Review'},[-200 200].*pi/180);
h4 = vector_plot(Sim_t,Sim_moments,MCU_desired_moments,{'M_x (N-m)', 'M_y (N-m)', 'M_z (N-m)', 'Body Moments Review'},[-0.2 0.2]);