...
unsigned char Setup(){
	...
	Setup_Bitmask |= SSD_setup_status | (BAR_setup_status<<1) | (IMU_setup_status<<2) | (MAG_setup_status<<3) | (LoRa_setup_status<<4);
	Setup_Timers();
	sei();
...
void Run(unsigned char Setup_Bitmask){
	...
	if (g_GPS_Read_Flag){
		...
		Navigation_Bitmask = SET_BIT(Navigation_Bitmask, 0, GPS_status);
	}
	
	if ((g_BAR_Read_Flag >= 3)&&(Setup_Bitmask & 0x02)){
		g_BAR_Read_Flag = 0;
		unsigned char BAR_status = Read_Bar(&Drone);
		Navigation_Bitmask = SET_BIT(Navigation_Bitmask, 1, BAR_status);
	}
	
	if ((g_MAG_Read_Flag >= 2)&&(Setup_Bitmask & 0x08)){
		g_MAG_Read_Flag = 0;
		unsigned char MAG_status = Read_Mag(&Drone);
		Navigation_Bitmask = SET_BIT(Navigation_Bitmask, 3, MAG_status);
	}
	
	if ((g_IMU_Read_Flag)&&(Setup_Bitmask & 0x04)){
		g_IMU_Read_Flag = 0;
		unsigned char IMU_status = Read_IMU(&Drone);
		Navigation_Bitmask = SET_BIT(Navigation_Bitmask, 2, IMU_status);
	}
	...
	if (g_LoRa_Flag){
		g_LoRa_Flag = 0;
		unsigned char LoRa_status = Read_LoRa(&Reference);
		Navigation_Bitmask = SET_BIT(Navigation_Bitmask, 4, LoRa_status);
	}
	
	if ((g_Print_Flag >= 50)&&(Setup_Bitmask & 0x01)){
		g_Print_Flag = 0;
		char buffer0[10];
		char buffer1[10];
		char buffer2[15];
		...
		//length_to_print = snprintf(buffer3, sizeof(buffer3), "%1.3f", Drone.Speed_over_ground);
		//Print_Page(3, buffer3, length_to_print);
	}
	
	// GUIDANCE //
	
	if (Navigation_Bitmask & 0x0F){
